...
{
    [ExecuteInEditMode]
    public class AutoCamWithOffset : PivotBasedCameraRig
    {
        ...
        void DebugNormalAngles(Transform trans)
        {
            ...
            Debug.Log(
                //$"Arctan2 {Mathf.Atan2(forward.x, forward.z) * Mathf.Rad2Deg}" +
                //$" Arctan {Mathf.Atan(forward.x) * Mathf.Rad2Deg}" +
                //$" forward {forward} up {up} right {right}" +
                //$" sin {Math.Sin(forward.x)} cos {Math.Cos(forward.x)}" +
